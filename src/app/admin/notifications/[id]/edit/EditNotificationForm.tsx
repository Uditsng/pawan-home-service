...
export function EditNotificationForm({
  ...
  // Deep Link States (Parsed on mount)
  ...
  // Audience States
  const [audienceType, setAudienceType] = useState(campaign.audience_type);
  const [selectedUserIds, setSelectedUserIds] = useState<string[]>(campaign.audience_filters?.userIds || []);
  const [userSearchTerm, setUserSearchTerm] = useState("");
  const [showUserDropdown, setShowUserDropdown] = useState(false);

  // Delivery & Expiry States
  const [deliveryOption, setDeliveryOption] = useState<"now" | "schedule" | "draft">(
  ...
  // Parse Initial Deep Link on Mount
  useEffect(() => {
    const dl = campaign.deep_link || "";
  ...
  const getResolvedDeepLink = () => {
    switch (deepLinkType) {
      ...
      case "booking_details": return "/customer/bookings/details";
  ...
  const submitForm = async () => {
    ...
    try {
      ...
      const result = await updateAdminNotification(campaign.id, campaignData);
      if (result) {
  ...
  const handleBroadcastConfirm = async () => {
    ...
    try {
      ...
      await updateAdminNotification(campaign.id, campaignData);
      ...
      <form onSubmit={handleSaveClick} className="lg:col-span-2 space-y-6">
        <Card variant="solid" className="p-6 space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="text-base font-bold text-primary font-headline font-semibold">Campaign Content</h2>
            <Badge variant="primary" className="text-[10px]">Editing Draft</Badge>
          </div>
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="e.g. Book any cleaning service this weekend..."
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              >
                ...
                <option value="high">High (Bypasses battery limits)</option>
              </select>
            </div>
          </div>
        </Card>
        ...
        <div className="flex justify-end gap-3 pt-2">
          ...
          >
            {loading ? "Saving..." : deliveryOption === "now" ? "Broadcast Now" : "Update Settings"}
          </Button>
        </div>
      </form>
            ...
            <div className="w-full bg-white/95 backdrop-blur-md rounded-2xl p-3 shadow-2xl border border-white/20 space-y-2 text-slate-900 animate-pulse">
              ...
              <div className="space-y-1">
                ...
                <p className="text-[11px] text-slate-600 font-medium leading-relaxed line-clamp-3">
                  {message || "Type the message body in the form to preview the layout..."}
                </p>
              </div>
            ...
            <div className="py-6 flex flex-col items-center justify-center space-y-4">
              ...
              {sendProgressStep === "failed" && (
                <>
                  ...
                  <p className="text-[11px] text-on-surface-variant/60">Please check server logs for details.</p>
                </>
              )}
            </div>
