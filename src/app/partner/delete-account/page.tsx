    ...
    <div className="bg-surface text-on-surface min-h-screen pb-24 lg:pb-12 flex flex-col font-body">
      ...
      <main className="flex-1 max-w-5xl mx-auto w-full px-4 sm:px-6 lg:px-8 pt-4 sm:pt-6 flex flex-col justify-center py-8 sm:py-16">
        ...
        <div className="max-w-md w-full rounded-3xl p-6 sm:p-8 text-center shadow-xs relative overflow-hidden bg-surface-container-lowest mx-auto border border-outline-variant/15">
          <div className="absolute -top-10 -right-10 w-32 h-32 bg-error/10 rounded-full blur-2xl pointer-events-none" />

          {/* Standard icon container */}
          <div className="w-16 h-16 bg-error/10 rounded-2xl flex items-center justify-center mx-auto mb-5 text-error">
            <span className="material-symbols-outlined text-3xl">no_accounts</span>
          </div>

          <span className="inline-flex items-center gap-1 bg-error/15 px-3 py-1 rounded-full text-[10px] sm:text-xs font-extrabold text-error mb-3 uppercase tracking-widest border border-error/30">
            Account Request
          </span>
          ...
          <div className="pt-4 border-t border-outline-variant/15 text-[11px] text-on-surface-variant/60 font-medium">
            We value your privacy and handle all data requests in accordance with platform policies.
          </div>
        </div>
      </main>
    </div>
