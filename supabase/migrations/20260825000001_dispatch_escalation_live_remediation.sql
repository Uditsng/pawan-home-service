-- ════════════════════════════════════════════════════════════════════════════
-- Migration: Dispatch Escalation Schema — Live Remediation
-- File: 20260825000001_dispatch_escalation_live_remediation.sql
-- Purpose:
--   The production database never received migration
--   20260823000001_dispatch_escalation_schema.sql. This file reapplies it,
--   fully idempotent, so it can be run safely on any environment state:
--     1. Adds bookings.dispatch_status / bookings.dispatch_locked_at.
--     2. Adds the chk_bookings_dispatch_status CHECK constraint.
--     3. Creates claim_dispatch_tier / confirm_dispatch_batch /
--        fail_dispatch_batch RPCs required by the Tier broadcast engine.
-- ════════════════════════════════════════════════════════════════════════════

-- 1. Missing booking columns
...
-- 2. Check constraint on dispatch_status
DO $$
BEGIN
  IF NOT EXISTS (
...
-- 3a. claim_dispatch_tier RPC (atomic two-phase tier claiming)
...
BEGIN
  ...
  IF p_target_tier = 1 THEN
    IF v_booking.broadcast_tier = 0 OR v_booking.broadcast_tier IS NULL OR v_booking.dispatch_status = 'failed' THEN
      UPDATE public.bookings
