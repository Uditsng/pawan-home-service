      ...
      <main className="max-w-7xl mx-auto px-4 md:px-6 pt-4 md:pt-6">
        ...
        <section className="mb-8 md:mb-12">
          <div className="grid grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3 md:gap-4">
            {availableServices.map((service) => {
              ...
              return (
                <Link href={`/customer/services/${catSlug}/${service.id}`} key={service.id} className="bg-surface-container-low p-3 md:p-5 rounded-xl flex flex-col items-center justify-center text-center hover:bg-surface-container-high transition-colors group cursor-pointer border border-outline-variant/10 shadow-sm">
        ...
        <section className="mb-8 md:mb-12">
          <div className="flex justify-between items-end mb-4 md:mb-6">
            ...
            {activeBookings.length > 0 && <Link href="/customer/bookings" className="text-primary text-xs md:text-sm font-bold">View History</Link>}
          </div>
              ...
              activeBookings.map((b) => (
                <div key={b.id} className="min-w-[250px] md:min-w-[280px] bg-surface-container-lowest p-4 md:p-5 rounded-xl shadow-[0_4px_12px_rgba(15,23,42,0.03)] border border-outline-variant/10 relative overflow-hidden group">
                  ...
                  <div className="flex justify-between items-center pt-3 md:pt-4 border-t border-surface-variant/30">
                    ...
                    <Link href={`/customer/bookings/${b.id}/tracking`} className="text-[10px] md:text-xs font-bold text-on-surface-variant hover:text-primary transition-colors flex items-center gap-1">
