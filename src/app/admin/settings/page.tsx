  ...
  return (
    <div className="space-y-4 sm:space-y-5 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div>
        ...
        <p className="text-on-surface-variant font-medium mt-0.5 opacity-70 text-xs sm:text-sm">Manage live service zones, pincodes, platform rules, and customer demand interest.</p>
      </div>

      {/* Database Schema Warning Banner */}
      {isSchemaError && (
        <div className="bg-amber-500/10 border border-amber-500/30 rounded-2xl p-4 shadow-xs flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
          <div className="flex items-start gap-3">
            <div className="w-10 h-10 bg-amber-500/20 rounded-xl flex items-center justify-center shrink-0">
              <span className="material-symbols-outlined text-amber-700">warning</span>
            </div>
            <div>
              <h4 className="text-xs font-black text-amber-800 uppercase tracking-tight">Database Schema Upgrade Required</h4>
          ...
          <div className="shrink-0 w-full sm:w-auto bg-amber-500/20 text-amber-800 font-black text-[10px] uppercase tracking-widest px-3 py-2 rounded-xl border border-amber-500/25 text-center">
            Schema Pending
          </div>
        </div>
      )}
      ...
      <SettingsConsole
        ...
        initialPartnerPayoutMin={String(settings.partnerPayoutMin)}
        initialPartnerPayoutsEnabled={settings.partnerPayoutsEnabled}
