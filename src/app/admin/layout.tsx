...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="bg-surface text-on-surface min-h-screen flex font-body selection:bg-secondary/30">
      ...
      {/* Desktop Sidebar */}
      <aside className="hidden lg:flex fixed left-0 top-0 h-screen w-72 bg-surface-container-lowest flex-col py-8 px-6 z-50 border-r border-outline-variant/30">
        <div className="mb-12">
          <Link href="/" className="flex items-center gap-4">
            <div className="w-12 h-12 bg-primary rounded-2xl flex items-center justify-center shadow-xl shadow-primary/20 group-hover:scale-105 transition-transform">
          ...
          {navLinks.map((link) => {
            ...
            return (
              <Link
                key={link.name}
                href={link.href}
                className={`flex items-center gap-4 px-5 py-3.5 rounded-2xl transition-all font-bold text-[14px] ${isActive
                  ? "bg-primary text-white shadow-xl shadow-primary/20 translate-x-2"
                  : "text-on-surface-variant hover:bg-surface-container-low hover:text-primary hover:translate-x-1"
                  }`}
              >
                <span className="material-symbols-outlined text-[20px]" style={isActive ? { fontVariationSettings: "'FILL' 1" } : {}}>
                  {link.icon}
        ...
        <div className="mt-10 pt-8 border-t border-outline-variant/20">
          <div className="flex items-center gap-4 p-3 rounded-2xl bg-surface-container-low border border-outline-variant/10">
            ...
            <div className="overflow-hidden">
              <p className="text-xs font-black text-primary truncate uppercase tracking-widest">Alex Rivera</p>
              <p className="text-[9px] text-on-surface-variant font-bold uppercase tracking-widest opacity-60">Global Manager</p>
            </div>
          </div>
        </div>
      </aside>
      ...
      <div className="flex-1 lg:ml-72 flex flex-col min-h-screen">
        ...
        {/* Content Canvas */}
        <main className="flex-1 p-6 md:p-10 bg-surface-dim/30">
          {children}
        </main>
      </div>
    </div>
