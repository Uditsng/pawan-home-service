...
/**
 * Migrate service catalog images from local codebase public folder to Supabase Storage.
 * Bypasses database RLS policies using service role privileges.
 */
export async function migrateServiceImagesAction() {
  await requireAdmin();
  ...
  try {
    // 1. Ensure services bucket exists
    ...
    if (!exists) {
      ...
      if (createError) {
        throw new Error(`Failed to create 'services' bucket: ${createError.message}`);
      }
      logs.push("Created storage bucket 'services' successfully.");
    } else {
      logs.push("Verified storage bucket 'services' exists.");
    }
    ...
    if (!services || services.length === 0) {
      logs.push("No services found in database to migrate.");
      return { success: true, processed: 0, migrated: 0, skipped: 0, failed: 0, logs };
    }

    // 3. Migrate each service image
    for (const service of services) {
      ...
      // If it starts with http and is already in public/services, it is already migrated
      ...
      if (!fs.existsSync(absolutePath)) {
        failed++;
        logs.push(`[${title}] File not found locally at public/${relativePath}.`);
        continue;
      }
      ...
      try {
        ...
        // Create clean filename: e.g. "bathroom-cleaning.png"
        ...
        migrated++;
        logs.push(`[${title}] Successfully migrated locally stored asset to Storage: ${publicUrl}`);
      } catch (err) {
        failed++;
        logs.push(`[${title}] Failed: ${(err as Error).message}`);
      }
    }
